...
async def receive_webhook(
    ...
    for entry in payload.entry:
        for change in entry.changes:
            try:
                async with db.begin():
                    _process_webhook_entry(change, db)
...
def _process_webhook_entry(change: WebhookChange, db: AsyncSession) -> None:
    value = change.value
    ...
    if value.messages:
        for message in value.messages:
            _process_message(message, value.contacts, db)

    if value.statuses:
        for status in value.statuses:
            _process_status(status, db)


def _process_message(message, contacts, db: AsyncSession) -> None:
...
def _process_status(status, db: AsyncSession) -> None:
    result = db.execute(
